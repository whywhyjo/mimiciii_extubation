  ...
  , max(
    ...
    , max(
      case when itemid is null or value is null then 0
        ...
        -- when itemid = 640 and value = 'Self Extubation' then 1 -- modified
      else 0
      end
      )
      as Extubated
    , max(
      ...
      as Unplanned -- modify selfextubation to unplaaned
from chartevents ce
...
select
  ...
  , 0 as OxygenTherapy
  , 1 as Extubated
  , case when (itemid = 225468 or itemid =225477) then 1 else 0 end as Unplanned -- modify selfextubation to unplaaned
